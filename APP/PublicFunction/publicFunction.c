/*************************************************************************************
* @file         publicFunction.c
* @author       Guangwei Huang
* @date         2018-09-06
* @attention     
* @Revision History
------------------------------------------------------------------------------------
* Version        Date         Author               Description
*-----------------------------------------------------------------------------------
* 00.01.00       2018-09-06   Guangwei Huang       Initial Version

*************************************************************************************/

/*************************************************************************************
* INCLUDES
*************************************************************************************/
#include "publicFunction.h"

/*************************************************************************************
* VARIABLE DEFINITION AND DECLARATION
*************************************************************************************/
const u8 Crc8H2FTable[256] ={
    
};

/*************************************************************************************
* FUNCTION PROTOTYPES
*************************************************************************************/


/* ============================================================================= *
* FUNCTION: Func_CalculateCRC8H2F(const u8 data[],s16 length)
* PURPOSE : 
* INPUT:    u8 data[],s16 length
* RETURN:   crc
* Author:   Guangwei Huang
* Date:     2018-07-12
* ============================================================================== */
u8 Func_CalculateCRC8H2F(const u8 data[],s16 length)
{
    s16 i;
    u8 c = 0u;
    u8 crc = 0u;
    crc = 0xFFu;
    for(i = 0; i < length; i++)
    {
        c = data[i];
        crc = Crc8H2FTable[crc^c];
    }
    crc ^= 0xFFu;
    return(crc);
}

bool Func_DelayTimeU16(u16 *timer, u16 delayTimeMs, u16 u16RunPeriodMs)
{
    bool result = FALSE;

    (*timer) += u16RunPeriodMs;
    if (*timer >= delayTimeMs)
    {
        *timer = delayTimeMs;
        result = TRUE;
    }

    return result;
}

bool Func_ReachTickerMaxU16(u16 *ticker, u16 tickerMax, u16 u16RunPeriodMs)
{
    bool result = FALSE;
    if (*ticker < tickerMax)
    {
        (*ticker) += u16RunPeriodMs;
    }
    else
    {
        result = TRUE;
    }
    return result;
}

bool Func_DelayTimeU32(u32 *timer, u32 delayTimeMs, u16 u16RunPeriodMs)
{
    bool result = FALSE;
    
    (*timer) += (u32)u16RunPeriodMs;
    if (*timer >= delayTimeMs)
    {
        (*timer) = delayTimeMs;
         result = TRUE;
    }
    return result;
}


/*********************************** END OF FILE ************************************/

